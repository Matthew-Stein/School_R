...
#Pretty plots
####
wi    <- 1/sqrt(meta$LRRv) #need to extract the weights first
meta$size  <- 0.5 + 3.0 * (wi - min(wi))/(max(wi) - min(wi)) #and bind the weights to the meta data frame
...
modelmean<-data.frame(common_m_lrr$b, common_m_lrr$se, 'Summary',5); names(modelmean)<-c('LRRy','LRRv','auth_ord',size); modelmean$author<-'Model Average'
prettymeta <- meta %>%
	mutate(Study=row.names(.)) %>%
	mutate(author = paste(genus,paper,Study,sep="_")) %>%
